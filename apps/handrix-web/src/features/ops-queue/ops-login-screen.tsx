...
import { createInternalSession, OpsAuthError } from './ops-auth-api'

type OpsLoginScreenProps = {
  onAuthenticated: (session: InternalSession) => void
}

export function OpsLoginScreen({ onAuthenticated }: OpsLoginScreenProps) {
  const [email, setEmail] = useState('ops@handrix.local')
  const [password, setPassword] = useState('ops-demo-pass')
    ...
    <main className="app-shell ops-page">
      <section className="ops-hero panel">
        ...
        <div className="hero-heading-row">
          <h1 className="hero-title hero-title--wide">Operations access</h1>
        </div>
        <p className="lede">
          Use an authorized staff account to enter the protected operations area.
        </p>
      </section>
      ...
      <section className="panel ops-login-panel">
        <div className="ops-login-panel__copy">
          ...
          <p className="helper-copy">
            This area is reserved for internal operations staff. Customer request data does not
        ...
        <form className="ops-login-form" onSubmit={handleSubmit}>
          ...
          <button className="primary-button" disabled={isSubmitting} type="submit">
            {isSubmitting ? 'Signing in…' : 'Enter operations area'}
          </button>
        </form>
      </section>
    </main>
